...
*/
function ordinare (pets){
  return pets.sort();
}
console.log(ordinare(pets));
/* ESERCIZIO 3
    Scrivi del codice per stampare nuovamente in console gli elementi dell'array "pets", questa volta in ordine invertito.
*/
function ordineInvertito (pets){
  return pets.reverse();
}
console.log(ordineInvertito(pets));
/* ESERCIZIO 4
    Scrivi del codice per spostare il primo elemento dall'array "pets" in ultima posizione.
*/
let primoElemento = pets.shift();
let ultimaPosizione = pets.push(primoElemento);
console.log(pets);
...
let v = 0;
while (v < numericArray.length){
  console.log(numericArray[v])
  v++
    if(numericArray[v]=== 32){
      break
    }
}
...
charactersArray.forEach(lettera => {
    switch (lettera) {
